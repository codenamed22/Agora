...
export default async function CohortDetailPage({
  params,
  searchParams,
}: Readonly<{ params: { id: string }; searchParams?: { status?: string; view?: string } }>) {
  await requireAdmin();
  ...
  const view = searchParams?.view === "edit" ? "edit" : "review";
  const activeStatus =
    STATUS_TABS.find((tab) => tab.key === searchParams?.status) ?? STATUS_TABS[0];

  // Only the Review tab renders applications, so skip both queries on Edit
  // (the findMany joins user + profile — wasted work otherwise).
  const [applications, counts] =
    view === "review"
      ? await Promise.all([
          prisma.application.findMany({
            where: { cohortId: cohort.id, status: activeStatus.status },
  ...
  return (
    <main className="app-shell wide-card workspace-shell">
      <section className="app-card workspace-card">
        ...
        <nav className="filter-tabs" aria-label="Cohort view">
          <a
            href={`/admin/cohort/${cohort.id}?view=review&status=${activeStatus.key}`}
            className={`filter-tab${view === "review" ? " is-active" : ""}`}
          >
            Review
          </a>
          <a
            href={`/admin/cohort/${cohort.id}?view=edit&status=${activeStatus.key}`}
            className={`filter-tab${view === "edit" ? " is-active" : ""}`}
          >
            Edit
          </a>
        </nav>
          ...
          <>
            {cohort.isActive ? (
              ...
              <p className="muted-line">
                Recruitment for this cohort has ended. Open a new year from the Cohorts page to
          ...
          <div className="portal-section">
            ...
            <nav className="filter-tabs" aria-label="Filter applications by status">
              {STATUS_TABS.map((tab) => (
                <a
                  key={tab.key}
                  href={`/admin/cohort/${cohort.id}?view=review&status=${tab.key}`}
                  className={`filter-tab${tab.key === activeStatus.key ? " is-active" : ""}`}
                >
                  ...
                  return (
                    <article className="application-row" key={application.id}>
                      <div>
                        ...
                        <p>
                          <strong>Batch:</strong>{" "}
                          {application.user.profile?.batch ?? "Not provided"} ·{" "}
